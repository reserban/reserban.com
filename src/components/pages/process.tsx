    ...
    <section className="pt-12 pb-20">
      <div className="container-none mx-auto max-w-[56rem] px-6 md:px-0">
        ...
        <Tabs className="relative" defaultValue="0">
          <TabsList className="mb-10 md:mb-4 grid mx-4  grid-cols-2 gap-x-2.5 gap-y-3 md:gap-y-2 bg-transparent p-0 lg:grid-cols-4">
            {integrations.map((item, index) => (
              <TabsTrigger
                ...
                } block cursor-pointer whitespace-normal border rounded-full border-gray-200 px-3 py-2 text-center transition-all duration-300 hover:bg-res-50 data-[state=active]:shadow-sm`}
              >
            ...
            <div className="p-1 pb-3">
              {integrations.map((item, index) => (
                <TabsContent
                  ...
                  className="m-0"
                >
                  <div className="overflow-hidden -mt-1 rounded-2xl h-[400px] relative">
                    <Image
                      src={isMobile ? item.mobileImage : item.image}
                      alt={`${item.title} illustration`}
                      fill
                      className="object-cover rounded-lg object-center transition-all duration-500"
